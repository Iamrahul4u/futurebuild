...
import { getUserCredits } from "@/app/actions/user.action";
import { useEffect, useState } from "react";
export default function CreditsButton({ userId }: { userId: string }) {
  const [credits, setCredits] = useState(0);
  useEffect(() => {
    const fetchCredits = async () => {
      const credits = await getUserCredits({ userId });
      if (typeof credits === "object" && "error" in credits) {
        setCredits(0);
        return;
      }
      setCredits(credits);
  ...
  return (
    <div className="sp">
      <button className="sparkle-button">
        ...
        <span className="text">{credits} Credits Left</span>
      </button>
